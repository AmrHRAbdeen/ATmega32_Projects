...
int main(void)
{
	...
	while(1){
		...
		LOC_u8I_RH=DHT22_u8Receive_Data();
		LOC_u8D_RH=DHT22_u8Receive_Data();
		HUM=(LOC_u8I_RH<<8)|(LOC_u8D_RH);

		LOC_u8I_Temp=DHT22_u8Receive_Data();
		LOC_u8D_Temp=DHT22_u8Receive_Data();
		TEMP=(LOC_u8I_Temp<<8)|(LOC_u8D_Temp);

		LOC_u8Check_Sum=DHT22_u8Receive_Data();

		if(LOC_u8I_RH+LOC_u8D_RH+LOC_u8I_Temp+LOC_u8D_Temp != LOC_u8Check_Sum){

			LCD_VidWriteCommand(LCD_CLEAR);
			LCD_VidWriteString((u8 *)"ERROR");


		}else{
			LCD_VidSetPosition(0,6);
			LCD_VidWriteNumber(HUM);


			LCD_VidSetPosition(1,7);
			LCD_VidWriteNumber(LOC_u8Check_Sum);
